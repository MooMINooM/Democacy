...
export const ui = {
    ...
    showAppointModal(mName) {
        ...
        govtParties.forEach(p => {
            ...
            candidates.forEach(l => {
                ...
                html += `<tr class="hover:bg-zinc-800/50 transition">
                    <td class="p-3 font-bold">${l.name}<div class="text-[9px] font-normal text-zinc-500">${trait.socio?.name || 'ไม่ระบุอาชีพ'}</div></td>
                    <td class="p-3"><div class="flex items-center gap-2"><div class="w-2 h-2 rounded-full" style="background:${l.party.color}"></div><span class="text-[10px]">${l.party.name}</span></div></td>
                    <td class="p-3 text-center"><div class="flex justify-center gap-1">${abilityIcon}${ideologyIcon}${goalIcon}</div></td>
                    <td class="p-3 text-center font-mono font-bold ${loyaltyColor}">${l.loyalty.toFixed(0)}%</td>
                    <td class="p-3 text-center"><button onclick="engine.appointMinister('${mName}', ${l.id}); document.getElementById('event-modal').classList.add('hidden'); gameClock.setSpeed(1);" class="bg-zinc-100 hover:bg-emerald-500 hover:text-white text-black px-3 py-1 rounded text-[10px] font-bold transition shadow-sm">แต่งตั้ง</button></td>
                </tr>`;
            });
        });
    ...
    showMPActionModal(id) {
        ...
        const lobbyCost = 2000000 * trait.socio.costMod; const cobraCost = 10000000 * trait.ability.costMod * trait.socio.costMod; const switchCost = 50000000 * trait.ability.costMod * trait.socio.costMod; const isMyParty = l.party.id === state.player.party.id;
        ...
        document.getElementById('event-desc').innerHTML = `
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-left font-sans mb-6 text-white">
                <div class="bg-black/40 p-4 rounded-xl border border-zinc-700 space-y-2">
                    ...
                    <div class="bg-zinc-800/50 p-2 rounded mt-2 flex items-center gap-3"><div class="w-8 h-8 rounded-full bg-zinc-700 flex items-center justify-center"><i class="fas ${trait.socio.icon} text-emerald-400"></i></div><div><div class="text-xs text-zinc-300">อาชีพ: <b>${trait.socio.name}</b></div><div class="text-[10px] text-zinc-500">Wealth Factor: x${trait.socio.costMod}</div></div></div>
                </div>
                <div class="bg-black/40 p-4 rounded-xl border border-zinc-700 space-y-2">
                    <div class="text-[10px] text-zinc-500 uppercase font-bold border-b border-zinc-800 pb-1 mb-2">คุณลักษณะพิเศษ</div>
                    <div class="flex items-center gap-2 text-xs"><i class="fas ${Data.TRAIT_ICONS?.[trait.ideology] || 'fa-circle'} text-blue-400 w-4 text-center"></i><span class="text-zinc-400">แนวคิด:</span> <span class="text-white">${trait.ideology}</span></div>
                    <div class="flex items-center gap-2 text-xs"><i class="fas ${Data.TRAIT_ICONS?.[trait.goal] || 'fa-circle'} text-purple-400 w-4 text-center"></i><span class="text-zinc-400">เป้าหมาย:</span> <span class="text-white">${trait.goal}</span></div>
                    <div class="bg-zinc-800/50 p-2 rounded mt-2 flex items-center gap-3"><div class="w-8 h-8 rounded-full bg-zinc-700 flex items-center justify-center"><i class="fas ${trait.ability.icon} text-orange-400"></i></div><div><div class="text-xs text-orange-300">ความสามารถ: <b>${trait.ability.name}</b></div><div class="text-[10px] text-zinc-500">${trait.ability.desc}</div></div></div>
                </div>
            </div>
            <div class="bg-zinc-900 p-4 rounded-xl border border-zinc-800 mb-4 flex justify-between items-center shadow-inner"><div class="text-xs text-zinc-400 uppercase tracking-widest">สถานะความภักดี (Loyalty)</div><div class="text-2xl font-black font-mono ${l.loyalty > 50 ? 'text-emerald-500' : 'text-red-500'}">${l.loyalty.toFixed(0)}%</div></div>
...
### 2. ไฟล์ `js/engine.js` (ฉบับ Clean)
```javascript
import { state } from './state.js';
import * as Data from './data.js';
...
export const engine = {
    ...
    lobbyIndividual(mpId) {
        ...
        const cost = 2000000 * mp.trait.socio.costMod; 
